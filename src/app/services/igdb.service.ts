...
export class IgdbService {
  ...
  public findLastGames(date: number, page:number): Observable<Game[]>{
    this.searching = ""
    this.platform = 0
    return this.httpClient.post<Game[]>(`${this.cors_api_host}${this.baseURL}`, `fields platforms.name, platforms.abbreviation, cover.url, genres.name, *; sort first_release_date desc; where first_release_date != null & first_release_date < ${date} & rating != null & category != (1, 2, 3, 5) & genres != null & cover != null & name != null; limit 6; offset ${(page - 1) * 6};`, {headers: {'Client-ID': this.clientID, Authorization: 'Bearer ' + this.authToken}})
  }

  public findLastGamesForPlatform(date: number, platformId: number, page:number): Observable<Game[]>{
    console.log(platformId)
    this.platform = platformId
    this.searching = ""
    return this.httpClient.post<Game[]>(this.cors_api_host + this.baseURL, "fields platforms.name, platforms.abbreviation, cover.url, genres.name, *; sort first_release_date desc; where first_release_date != null & first_release_date < " + date + " & rating != null & platforms = (" + platformId + ") & genres != null & category != (1, 2, 3, 5) & cover != null; limit 6; offset " + (page - 1) * 6 + ";", {headers: {'Client-ID': this.clientID, Authorization: 'Bearer ' + this.authToken}})
  }
  ...
  public findSearchedGames(search: string, page:number){
    ...
    this.platform = 0
    return this.httpClient.post<Game[]>(this.cors_api_host + this.baseURL, 'fields platforms.name, cover.url, genres.name, *; limit 6; offset ' + (page - 1) * 6 + '; search "' + search + '" ;', {headers: {'Client-ID': this.clientID, Authorization: 'Bearer ' + this.authToken}})
  }
  ...
  public changePageNumber(date: number, page:number): Observable<Game[]>{
    if(this.platform === 0 && this.searching === '')
      ...
      return this.findLastGamesForPlatform(date, this.platform, page)
  }
  ...
  public getPlatformName(id: number): Observable<any>{
    ...
    return this.httpClient.post<any>(apiUrl, 'fields *; where id = ' + id +'; limit 1;', {headers: {'Client-ID': this.clientID, Authorization: 'Bearer ' + this.authToken}})
