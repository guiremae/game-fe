...
export class ShareComponent {
  constructor(
    ...
    iconRegistry.addSvgIconLiteral(
      'telegram',
      sanitizer.bypassSecurityTrustHtml(TELEGRAM_ICON)
    );
  }

  shareByEmail() {
    const subject = '¡Mira esta lista!';
    const body = `Te comparto esta lista: ${this.data.url}`;
  ...
  shareByWhatsApp() {
    const message = `Te comparto esta lista: ${this.data.url}`;
    const whatsappLink = `https://wa.me/?text=${encodeURIComponent(message)}`;
    window.open(whatsappLink, '_blank');
  }

  // Método para compartir por Telegram
  shareByTelegram() {
    const message = `Te comparto esta lista: ${this.data.url}`;
